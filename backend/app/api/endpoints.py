...
def process_estimate_background(estimate_id: int, file_bytes: bytes, impact_zone: str, db: Session):
    # 1. Extract Text
    ...
    # Reload line items to get their DB IDs
    db_line_items = db.query(DBLineItem).filter(DBLineItem.estimate_id == estimate_id).all()
...
async def upload_estimate(
    ...
    background_tasks.add_task(process_estimate_background, db_estimate.id, file_bytes, impact_zone, db)
